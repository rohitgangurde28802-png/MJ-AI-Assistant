 ...
 *   4. gemini-1.0-pro (quaternary)
 *   5. Smart offline fallback (always works)
 *
...
""".trimIndent()
     ...
     */
    fun fetchResponse(context: Context, userText: String, callback: (String) -> Unit) {
        val prefs = context.getSharedPreferences("mj_prefs", Context.MODE_PRIVATE)
        val customKey = prefs.getString("gemini_api_key", "")
        val activeKey = if (!customKey.isNullOrBlank()) customKey else apiKey

        // Quick check: if API key is clearly invalid, go to offline fallback immediately
        if (activeKey.isBlank() || activeKey == "YOUR_API_KEY_HERE") {
            Log.w(TAG, "API key not configured — using offline fallback")
            callback(getOfflineFallback(userText))
            return
        }
        attemptWithModel(context, userText, callback, modelIndex = currentModelIndex, retryCount = 0, activeKey = activeKey)
    }
    ...
    private fun attemptWithModel(
        ...
        val endpoint = "$BASE_URL/$model:generateContent?key=$activeKey"
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                ...
                if (retryCount < MAX_RETRIES) {
                    ...
                    attemptWithModel(context, userText, callback, modelIndex, retryCount + 1, activeKey)
                } else {
                    // Network failed — try next model
                    attemptWithModel(context, userText, callback, modelIndex + 1, 0, activeKey)
                }
            }
